...
public class rgk_6535MeV_p_gamma_correction {
	...
	static IndexedList<H1F> histGroups_X_eg_m_theta_bin = new IndexedList<H1F>(2);
	static IndexedList<H1F> histGroups_p_cor_X_eg_m_theta_bin = new IndexedList<H1F>(2);
	
	public static void histos_X_eg_m_vs_theta_bin() {
		for(int seci = 0; seci < 6; seci++) {
			for(int thetabin = 0; thetabin < 9; thetabin++) {
				H1F hX_eg_m_vs_theta_bin = new H1F("hX_eg_m_vs_theta_bin", "hX_eg_m_vs_theta_bin", 100, 0, 2);
				histGroups_X_eg_m_theta_bin.add(hX_eg_m_vs_theta_bin, seci, thetabin);
				H1F hp_cor_X_eg_m_theta_bin = new H1F("hp_cor_X_eg_m_theta_bin", "hp_cor_X_eg_m_theta_bin", 100, 0, 2);
				histGroups_p_cor_X_eg_m_theta_bin.add(hp_cor_X_eg_m_theta_bin, seci, thetabin);
			}
		}
	}
	
	static IndexedList<H1F> histGroups_dp = new IndexedList<H1F>(2);
	...
	public static void histos_dp() {
		for(int ihistsectordp = 0; ihistsectordp < 6; ihistsectordp++) {
			for(int ihistthetadp = 0; ihistthetadp < 9; ihistthetadp++) {
				...
				histGroups_dp.add(hdp, ihistsectordp, ihistthetadp);
			}
		}
	}
	
	static H2F hdp_vs_p_gamma_samp1 = new H2F("hdp_vs_p_gamma_samp1", "hdp_vs_p_gamma_samp1", 200, 0.5, 6.5, 200, 0.75, 2.75);
	static H2F hdp_vs_p_gamma_samp2 = new H2F("hdp_vs_p_gamma_samp2", "hdp_vs_p_gamma_samp2", 200, 0.5, 3.5, 200, 0.75, 2.75);
				...
				if(event.hasBank("REC::Calorimeter"))
				{
					...
					for(int l = 0; l < reccal.rows(); l++)
					{
						...
						if(pindex == gamma_pindex && detector == 7 && layer == 1)
						{
							...
							double a0 = thetagammaCorFac.getItem(gamma_sector)[0];
							double a1 = thetagammaCorFac.getItem(gamma_sector)[1];
							double a2 = thetagammaCorFac.getItem(gamma_sector)[2];
							double a3 = thetagammaCorFac.getItem(gamma_sector)[3];
							double a4 = thetagammaCorFac.getItem(gamma_sector)[4];
							...
							pcf = (a4*theta_deg_gamma*theta_deg_gamma*theta_deg_gamma*theta_deg_gamma)
									+(a3*theta_deg_gamma*theta_deg_gamma*theta_deg_gamma)+(a2*theta_deg_gamma*theta_deg_gamma)
									+(a1*theta_deg_gamma)+a0;
				...
				&& Q2 > 1 && W > 2)
		{
			...
			if(-t_cal.mass2() > 0.938272*0.938272*x_B*x_B/(1-x_B+x_B*0.938272*0.938272/Q2*Q2))
			{
				...
				for(int thbin = 0; thbin < 9; thbin++)
				{
					if(theta_deg_gamma > theta_bnd[thbin] && theta_deg_gamma <= theta_bnd[thbin+1])
					{
						...
						histGroups_X_eg_m_theta_bin.getItem(gamma_sector-1, thbin).fill(p_X_eg.mass());
						histGroups_dp.getItem(gamma_sector-1, thbin).fill(dp);
						if(thbin == 0) hdp_vs_p_gamma_samp1.fill(p_gamma.p(), dp);
						if(thbin == 8) hdp_vs_p_gamma_samp2.fill(p_gamma.p(), dp);
						break;
					}
				}
			}
			...
			if(-p_cor_t_cal.mass2() > 0.938272*0.938272*x_B*x_B/(1-x_B+x_B*0.938272*0.938272/Q2*Q2))
			{
				...
				for(int thbin = 0; thbin < 9; thbin++)
				{
					if(theta_deg_gamma > theta_bnd[thbin] && theta_deg_gamma <= theta_bnd[thbin+1])
					{
						histGroups_p_cor_X_eg_m_theta_bin.getItem(gamma_sector-1, thbin).fill(p_cor_p_X_eg.mass());
						break;
					}
				}
			}
	...
	public static void main(String[] args) {
		...
		IndexedList<double[]> thetagammaCorFac = new IndexedList<>(1);
		double[] thetagammaCorFac_sec1 = new double[] {1.1403061169110182, -0.020018206498366747, 0.0014861481062021786, 
														-4.043951749697515E-5, 4.430293884019251E-7};
		double[] thetagammaCorFac_sec2 = new double[] {1.2335159373847537, -0.0580600345495922, 0.005472463061323651, 
														-1.9619096057576157E-4, 2.54187843127528E-6};
		double[] thetagammaCorFac_sec3 = new double[] {1.3026977123493741, -0.07657530475577023, 0.0072627715388385305, 
														-2.675184793656458E-4, 3.501458802553962E-6};
		double[] thetagammaCorFac_sec4 = new double[] {1.0543988159408098, -0.00523846683681521, 8.118334193891186E-4, 
														-2.7960541564573504E-5, 3.154975009059783E-7};
		double[] thetagammaCorFac_sec5 = new double[] {1.1572857855235064, -0.03663699195554548, 0.0038170780457466025, 
														-1.4410275359095118E-4, 1.9370751495848373E-6};
		double[] thetagammaCorFac_sec6 = new double[] {1.277878887750518, -0.06262899556780079, 0.0057344312406990006, 
														-2.0360548284189487E-4, 2.5799162814241727E-6};
		
		thetagammaCorFac.add(thetagammaCorFac_sec1, 1);
		thetagammaCorFac.add(thetagammaCorFac_sec2, 2);
		thetagammaCorFac.add(thetagammaCorFac_sec3, 3);
		thetagammaCorFac.add(thetagammaCorFac_sec4, 4);
		thetagammaCorFac.add(thetagammaCorFac_sec5, 5);
		thetagammaCorFac.add(thetagammaCorFac_sec6, 6);
	...
	// DVCS	
		...
		JFrame framegammaeloss = new JFrame("#gamma #deltap vs. p");
		framegammaeloss.setSize(1000, 500);
		EmbeddedCanvas cangammaeloss = new EmbeddedCanvas();
		framegammaeloss.add(cangammaeloss);
		framegammaeloss.setLocationRelativeTo(null);
		framegammaeloss.setVisible(true);
		cangammaeloss.divide(2, 1);
		cangammaeloss.cd(0);
		cangammaeloss.setFont("Arial");
		hdp_vs_p_gamma_samp1.setTitle("#gamma #deltap vs. p");
		hdp_vs_p_gamma_samp1.setTitleX("p [GeV]");
		hdp_vs_p_gamma_samp1.setTitleY("#deltap [GeV]");
		cangammaeloss.getPad(0).setTitleFontSize(32);
		cangammaeloss.getPad(0).setAxisTitleFontSize(32);
		cangammaeloss.getPad(0).setAxisLabelFontSize(24);
		cangammaeloss.draw(hdp_vs_p_gamma_samp1, "same");
		cangammaeloss.cd(1);
		cangammaeloss.setFont("Arial");
		hdp_vs_p_gamma_samp2.setTitle("#gamma #deltap vs. p");
		hdp_vs_p_gamma_samp2.setTitleX("p [GeV]");
		hdp_vs_p_gamma_samp2.setTitleY("#deltap [GeV]");
		cangammaeloss.getPad(1).setTitleFontSize(32);
		cangammaeloss.getPad(1).setAxisTitleFontSize(32);
		cangammaeloss.getPad(1).setAxisLabelFontSize(24);
		cangammaeloss.draw(hdp_vs_p_gamma_samp2, "same");
	/*
		for(int canhg2i = 0; canhg2i < 9; canhg2i++){
			...
			canhg2.draw(histGroups_X_eg_m_vs_theta.getItem(canhg2i));
		}
		...
		for(int canhg2i = 0; canhg2i < 9; canhg2i++){
			...
			histGroups_X_eg_m_theta_bin.getItem(canhg2i).setTitleX("#phi [#degree]");
	...
	*/
		...
		*/
		...
		IndexedList<Double> meanXegm = new IndexedList<>(2);
		IndexedList<Double> sigmaXegm = new IndexedList<>(2);
		
		IndexedList<Double> meanpcorXegm = new IndexedList<>(2);
		IndexedList<Double> sigmapcorXegm = new IndexedList<>(2);
		...
		for(int seci = 0; seci < 6; seci++)
		{
			for(int theta_bini = 0; theta_bini < 9; theta_bini++)
			{
				fpm = new F1D("fpm", "[amp]*gaus(x,[mean],[sigma])",
								(histGroups_X_eg_m_theta_bin.getItem(seci, theta_bini).getMaximumBin()*0.02-0.45),
								(histGroups_X_eg_m_theta_bin.getItem(seci, theta_bini).getMaximumBin()*0.02+0.35));
				fpm.setParameter(0, histGroups_X_eg_m_theta_bin.getItem(seci, theta_bini).getMax());
				fpm.setParameter(1, histGroups_X_eg_m_theta_bin.getItem(seci, theta_bini).getMaximumBin()*0.02);
				fpm.setParameter(2, histGroups_X_eg_m_theta_bin.getItem(seci, theta_bini).getRMS()/2);
				fpm.setParLimits(2, histGroups_X_eg_m_theta_bin.getItem(seci, theta_bini).getRMS()/4,
									histGroups_X_eg_m_theta_bin.getItem(seci, theta_bini).getRMS());
				DataFitter.fit(fpm, histGroups_X_eg_m_theta_bin.getItem(seci, theta_bini), "Q");
				meanXegm.add(fpm.getParameter(1), seci, theta_bini);
				sigmaXegm.add(fpm.getParameter(2), seci, theta_bini);
				fpm = new F1D("fpm", "[amp]*gaus(x,[mean],[sigma])",
								(histGroups_p_cor_X_eg_m_theta_bin.getItem(seci, theta_bini).getMaximumBin()*0.02-0.45),
								(histGroups_p_cor_X_eg_m_theta_bin.getItem(seci, theta_bini).getMaximumBin()*0.02+0.35));
				fpm.setParameter(0, histGroups_p_cor_X_eg_m_theta_bin.getItem(seci, theta_bini).getMax());
				fpm.setParameter(1, histGroups_p_cor_X_eg_m_theta_bin.getItem(seci, theta_bini).getMaximumBin()*0.02);
				fpm.setParameter(2, histGroups_p_cor_X_eg_m_theta_bin.getItem(seci, theta_bini).getRMS()/2);
				fpm.setParLimits(2, histGroups_p_cor_X_eg_m_theta_bin.getItem(seci, theta_bini).getRMS()/4,
									histGroups_p_cor_X_eg_m_theta_bin.getItem(seci, theta_bini).getRMS());
				DataFitter.fit(fpm, histGroups_p_cor_X_eg_m_theta_bin.getItem(seci, theta_bini), "Q");
				meanpcorXegm.add(fpm.getParameter(1), seci, theta_bini);
				sigmapcorXegm.add(fpm.getParameter(2), seci, theta_bini);
			}
		}
		
		IndexedList<GraphErrors> histerrGroups_X_eg_m_vs_theta = new IndexedList<GraphErrors>(1);
		IndexedList<GraphErrors> histerrGroups_p_cor_X_eg_m_vs_theta = new IndexedList<GraphErrors>(1);
		
		for(int seci = 0; seci < 6; seci++) {
			GraphErrors hX_eg_m_vs_theta = new GraphErrors();
			histerrGroups_X_eg_m_vs_theta.add(hX_eg_m_vs_theta, seci);
			GraphErrors hp_cor_X_eg_m_vs_theta = new GraphErrors();
			histerrGroups_p_cor_X_eg_m_vs_theta.add(hp_cor_X_eg_m_vs_theta, seci);
				
			for(int theta_bini = 0; theta_bini < 9; theta_bini++)
			{
				if(meanXegm.hasItem(seci, theta_bini) && sigmaXegm.hasItem(seci, theta_bini))
					histerrGroups_X_eg_m_vs_theta.getItem(seci).addPoint(theta_ba[theta_bini],
																			meanXegm.getItem(seci, theta_bini),
																			0, sigmaXegm.getItem(seci, theta_bini));
				if(meanpcorXegm.hasItem(seci, theta_bini) && sigmapcorXegm.hasItem(seci, theta_bini))
					histerrGroups_p_cor_X_eg_m_vs_theta.getItem(seci).addPoint(theta_ba[theta_bini],
																								meanpcorXegm.getItem(seci, theta_bini),
																								0, sigmapcorXegm.getItem(seci, theta_bini));
			}
		}
		
		JFrame frameXegmthetabin = new JFrame("M_X_(ep->e'#gamma) vs. #theta Binned");
		frameXegmthetabin.setSize(1500, 1000);
		EmbeddedCanvas canXegmthetabin = new EmbeddedCanvas();
		frameXegmthetabin.add(canXegmthetabin);
		frameXegmthetabin.setLocationRelativeTo(null);
		frameXegmthetabin.setVisible(true);
		canXegmthetabin.divide(3, 2);
		for(int seci = 0; seci < 6; seci++){
			canXegmthetabin.cd(seci);
			canXegmthetabin.setFont("Arial");
			histerrGroups_X_eg_m_vs_theta.getItem(seci).setTitle("e^- Hit: M_X_(ep->e'#gamma) vs. #theta");
			histerrGroups_X_eg_m_vs_theta.getItem(seci).setTitleX("#phi [#degree]");
			histerrGroups_X_eg_m_vs_theta.getItem(seci).setTitleY("Sector " + (seci+1) + " M_X : #mu +/- #sigma [GeV]");
			histerrGroups_X_eg_m_vs_theta.getItem(seci).setMarkerSize(5);
			histerrGroups_X_eg_m_vs_theta.getItem(seci).setMarkerStyle(1);
			histerrGroups_X_eg_m_vs_theta.getItem(seci).setLineThickness(1);
			canXegmthetabin.getPad(seci).setTitleFontSize(32);
			canXegmthetabin.getPad(seci).setAxisTitleFontSize(32);
			canXegmthetabin.getPad(seci).setAxisLabelFontSize(24);
			canXegmthetabin.getPad(seci).setAxisRange(5, 35, 0, 2);
			histerrGroups_p_cor_X_eg_m_vs_theta.getItem(seci).setMarkerSize(5);
			histerrGroups_p_cor_X_eg_m_vs_theta.getItem(seci).setMarkerStyle(1);
			histerrGroups_p_cor_X_eg_m_vs_theta.getItem(seci).setMarkerColor(2);
			histerrGroups_p_cor_X_eg_m_vs_theta.getItem(seci).setLineColor(2);
			histerrGroups_p_cor_X_eg_m_vs_theta.getItem(seci).setLineThickness(1);
			canXegmthetabin.draw(histerrGroups_X_eg_m_vs_theta.getItem(seci), "same");
			canXegmthetabin.draw(histerrGroups_p_cor_X_eg_m_vs_theta.getItem(seci), "same");
		}
		
		int setsector = 1;
		
		double f00dp_rb = 0.2;
		double f00dp_lb = 0.15;
		
		for(int theta_bini = 0; theta_bini < 9; theta_bini++)
		{
			JFrame framedpbin = new JFrame("#deltap Binned");
			framedpbin.setSize(500, 500);
			...
			canhgdp.divide(1, 1);
				canhgdp.cd(theta_bini);
				canhgdp.setFont("Arial");
				histGroups_dp.getItem(setsector-1, theta_bini).setTitle("Sector " + setsector + " e^-: " + theta_bnd[theta_bini]
																					+ " #degree < #theta <= " + theta_bnd[theta_bini+1] + " #degree");
				histGroups_dp.getItem(setsector-1, theta_bini).setTitleX("#deltap");
				histGroups_dp.getItem(setsector-1, theta_bini).setTitleY("Counts");
				histGroups_dp.getItem(setsector-1, theta_bini).setOptStat(10);
				canhgdp.getPad(0).setTitleFontSize(32);
				canhgdp.getPad(0).setAxisTitleFontSize(32);
				canhgdp.getPad(0).setAxisLabelFontSize(24);
				canhgdp.getPad(0).setStatBoxFontSize(18);
				canhgdp.draw(histGroups_dp.getItem(setsector-1, theta_bini));
				F1D f00dp = new F1D("f00dp", "[amp]*gaus(x,[mean],[sigma])",
									0.25+(histGroups_dp.getItem(setsector-1, theta_bini).getMaximumBin()*(2.25-0.25)/200)-f00dp_lb,
									0.25+(histGroups_dp.getItem(setsector-1, theta_bini).getMaximumBin()*(2.25-0.25)/200)+f00dp_rb);
				f00dp.setParameter(0, histGroups_dp.getItem(setsector-1, theta_bini).getMax());
				f00dp.setParameter(1, 0.25+(histGroups_dp.getItem(setsector-1, theta_bini).getMaximumBin()*(2.25-0.25)/200));
				f00dp.setParameter(2, 0.07);
				DataFitter.fit(f00dp, histGroups_dp.getItem(setsector-1, theta_bini), "Q");
		...
		for(int theta_bini = 0; theta_bini < 9; theta_bini++)
		{
			...
			framecorXegmbin.setSize(500, 500);
			...
			canpcorXegmbin.divide(1, 1);
			canpcorXegmbin.cd(0);
			canpcorXegmbin.setFont("Arial");
			histGroups_p_cor_X_eg_m_theta_bin.getItem(setsector-1, theta_bini).setTitle("Sector " + setsector + " e^-: " + theta_bnd[theta_bini]
																							+ " #degree < #theta <= " + theta_bnd[theta_bini+1] + " #degree");
			histGroups_p_cor_X_eg_m_theta_bin.getItem(setsector-1, theta_bini).setTitleX("#deltap");
			histGroups_p_cor_X_eg_m_theta_bin.getItem(setsector-1, theta_bini).setTitleY("Counts");
			histGroups_p_cor_X_eg_m_theta_bin.getItem(setsector-1, theta_bini).setOptStat(10);
			canpcorXegmbin.getPad(0).setTitleFontSize(32);
			canpcorXegmbin.getPad(0).setAxisTitleFontSize(32);
			canpcorXegmbin.getPad(0).setAxisLabelFontSize(24);
			canpcorXegmbin.getPad(0).setStatBoxFontSize(18);
			canpcorXegmbin.draw(histGroups_p_cor_X_eg_m_theta_bin.getItem(setsector-1, theta_bini));
			fpm = new F1D("fpm", "[amp]*gaus(x,[mean],[sigma])", (histGroups_p_cor_X_eg_m_theta_bin.getItem(setsector-1, theta_bini)
								.getMaximumBin()*0.02-0.45), (histGroups_p_cor_X_eg_m_theta_bin.getItem(setsector-1, theta_bini)
								.getMaximumBin()*0.02+0.35));
			fpm.setParameter(0, histGroups_p_cor_X_eg_m_theta_bin.getItem(setsector-1, theta_bini).getMax());
			fpm.setParameter(1, histGroups_p_cor_X_eg_m_theta_bin.getItem(setsector-1, theta_bini).getMaximumBin()*0.02);
			fpm.setParameter(2, histGroups_p_cor_X_eg_m_theta_bin.getItem(setsector-1, theta_bini).getRMS()/2);
			fpm.setParLimits(2, histGroups_p_cor_X_eg_m_theta_bin.getItem(setsector-1, theta_bini).getRMS()/4,
									histGroups_p_cor_X_eg_m_theta_bin.getItem(setsector-1, theta_bini).getRMS());
			DataFitter.fit(fpm, histGroups_p_cor_X_eg_m_theta_bin.getItem(setsector-1, theta_bini), "Q");
			fpm.setLineColor(2);
			fpm.setLineWidth(3);
			fpm.setOptStat(11110);
		...
		for(int theta_bini = 0; theta_bini < 9; theta_bini++)
		{
			JFrame frameXegmbin = new JFrame("M_X_(ep->e'#gamma) Binned");
			frameXegmbin.setSize(500, 500);
			EmbeddedCanvas canpXegmbin = new EmbeddedCanvas();
			frameXegmbin.add(canpXegmbin);
			frameXegmbin.setLocationRelativeTo(null);
			frameXegmbin.setVisible(true);
			canpXegmbin.divide(1, 1);
			canpXegmbin.cd(0);
			canpXegmbin.setFont("Arial");
			histGroups_X_eg_m_theta_bin.getItem(setsector-1, theta_bini).setTitle("Sector " + setsector + " e^-: " + theta_bnd[theta_bini]
																							+ " #degree < #theta <= " + theta_bnd[theta_bini+1] + " #degree");
			histGroups_X_eg_m_theta_bin.getItem(setsector-1, theta_bini).setTitleX("#deltap");
			histGroups_X_eg_m_theta_bin.getItem(setsector-1, theta_bini).setTitleY("Counts");
			histGroups_X_eg_m_theta_bin.getItem(setsector-1, theta_bini).setOptStat(10);
			canpXegmbin.getPad(0).setTitleFontSize(32);
			canpXegmbin.getPad(0).setAxisTitleFontSize(32);
			canpXegmbin.getPad(0).setAxisLabelFontSize(24);
			canpXegmbin.getPad(0).setStatBoxFontSize(18);
			canpXegmbin.draw(histGroups_X_eg_m_theta_bin.getItem(setsector-1, theta_bini));
			fpm = new F1D("fpm", "[amp]*gaus(x,[mean],[sigma])", (histGroups_X_eg_m_theta_bin.getItem(setsector-1, theta_bini)
								.getMaximumBin()*0.02-0.45), (histGroups_X_eg_m_theta_bin.getItem(setsector-1, theta_bini)
								.getMaximumBin()*0.02+0.35));
			fpm.setParameter(0, histGroups_X_eg_m_theta_bin.getItem(setsector-1, theta_bini).getMax());
			fpm.setParameter(1, histGroups_X_eg_m_theta_bin.getItem(setsector-1, theta_bini).getMaximumBin()*0.02);
			fpm.setParameter(2, histGroups_X_eg_m_theta_bin.getItem(setsector-1, theta_bini).getRMS()/2);
			fpm.setParLimits(2, histGroups_X_eg_m_theta_bin.getItem(setsector-1, theta_bini).getRMS()/4,
									histGroups_X_eg_m_theta_bin.getItem(setsector-1, theta_bini).getRMS());
			DataFitter.fit(fpm, histGroups_X_eg_m_theta_bin.getItem(setsector-1, theta_bini), "Q");
			fpm.setLineColor(2);
			fpm.setLineWidth(3);
			fpm.setOptStat(11110);
			canpXegmbin.draw(fpm, "same");
		}
		
		IndexedList<Double> mean = new IndexedList<>(2);
		IndexedList<Double> sigma = new IndexedList<>(2);
		...
		for(int secdpi = 0; secdpi < 6; secdpi++)
		{
			...
			{
				F1D f00dp = new F1D("f00dp", "[amp]*gaus(x,[mean],[sigma])",
									0.25+(histGroups_dp.getItem(secdpi, theta_bini).getMaximumBin()*(2.25-0.25)/200)-f00dp_lb,
									0.25+(histGroups_dp.getItem(secdpi, theta_bini).getMaximumBin()*(2.25-0.25)/200)+f00dp_rb);
				f00dp.setParameter(0, histGroups_dp.getItem(secdpi, theta_bini).getMax());
				f00dp.setParameter(1, 0.25+(histGroups_dp.getItem(secdpi, theta_bini).getMaximumBin()*(2.25-0.25)/200));
				f00dp.setParameter(2, 0.07);
				DataFitter.fit(f00dp, histGroups_dp.getItem(secdpi, theta_bini), "Q");
				mean.add(f00dp.getParameter(1), secdpi, theta_bini);
				sigma.add(f00dp.parameter(1).error(), secdpi, theta_bini);
			}
		}
		...
		for(int seci = 0; seci < 6; seci++)
		{
			for(int thetabi = 0; thetabi < 9; thetabi++)
			{
				...
				List<Double> thetabci = new ArrayList<>();
			
				if(mean.getItem(seci, thetabi) > 0.75 && mean.getItem(seci, thetabi) < 1.5
						&& Math.abs(sigma.getItem(seci, thetabi)) < 0.3)
				{
					meani.add(mean.getItem(seci, thetabi));
					sigmai.add(sigma.getItem(seci, thetabi));
					thetabci.add(theta_ba[thetabi]);
				}
				
				FCN4 theFCN4 = new FCN4(thetabci, meani, sigmai);
					
				MnUserParameters upar = new MnUserParameters();
				upar.add("a0", 0.9, 0.1);
				upar.add("a1", 0, 0.01);
				upar.add("a2", 0, 0.001);
				upar.add("a3", 0, 0.0001);
				upar.add("a4", 0, 0.00001);
				
				System.out.println("Initial parameters: "+upar); 
			    
			    System.out.println("start migrad");
				MnMigrad migrad = new MnMigrad(theFCN4, upar); 
			    FunctionMinimum min = migrad.minimize(); 
			    if(!min.isValid()) 
			    { 
			    	//try with higher strategy 
				    	System.out.println("FM is invalid, try with strategy = 2."); 
				    	MnMigrad migrad2 = new MnMigrad(theFCN4, min.userState(), new MnStrategy(2)); 
			    ...
			    {
			    	thetaCorFac.add(min.userParameters().value(cfi), seci, cfi);
			    }
			    for(int cfei = 5; cfei < 10; cfei++)
			    {
			    	thetaCorFac.add(min.userParameters().error(cfei-5), seci, cfei);
			    }
			    thetaCorFac.add(Math.sqrt(min.fval()), seci, 10);
			    */
			}
		}
		
		IndexedList<GraphErrors> histerrGroups_dp_vs_theta = new IndexedList<GraphErrors>(1);
		
		for(int seci = 0; seci < 6; seci++)
		{
			GraphErrors hedp_vs_theta = new GraphErrors();
			histerrGroups_dp_vs_theta.add(hedp_vs_theta, seci);
			for(int thetadpi = 0; thetadpi < 9; thetadpi++)
			{
				if(mean.hasItem(seci, thetadpi) && sigma.hasItem(seci, thetadpi))
				histerrGroups_dp_vs_theta.getItem(seci).addPoint(theta_ba[thetadpi], mean.getItem(seci, thetadpi),
																			0, sigma.getItem(seci, thetadpi));
			}
		}
		
		JFrame framedpphibin = new JFrame("#deltap vs. #theta Binned");
		...
		for(int seci = 0; seci < 6; seci++)
		{
			...
			histerrGroups_dp_vs_theta.getItem(seci).setTitle("Sector " + (seci+1) + " e^-: #deltap vs. #theta");
			histerrGroups_dp_vs_theta.getItem(seci).setTitleX("#phi_midsector [#degree]");
			histerrGroups_dp_vs_theta.getItem(seci).setTitleY("#deltap");
			histerrGroups_dp_vs_theta.getItem(seci).setMarkerSize(5);
			histerrGroups_dp_vs_theta.getItem(seci).setMarkerStyle(1);
			histerrGroups_dp_vs_theta.getItem(seci).setLineThickness(1);
			...
			canhgedp.getPad(seci).setAxisRange(5, 35, 1, 1.3);
			canhgedp.draw(histerrGroups_dp_vs_theta.getItem(seci));
			F1D fdp = new F1D("fdp", "[a0]+([a1]*x)+([a2]*x*x)+([a3]*x*x*x)+([a4]*x*x*x*x)", 0, 35);
			fdp.setParameter(0, 0);
			fdp.setParameter(1, 0);
			fdp.setParameter(2, 0);
			fdp.setParLimits(2, 0, 1);
			fdp.setParameter(3, 0);
			fdp.setParameter(4, 0);
			DataFitter.fit(fdp, histerrGroups_dp_vs_theta.getItem(seci), "Q");
			fdp.setLineColor(2);
			fdp.setLineWidth(3);
			fdp.setOptStat(111110);
			canhgedp.draw(fdp, "same");
			for(int cfi = 0; cfi < 5; cfi++)
		    {
		    	thetaCorFac.add(fdp.getParameter(cfi), seci, cfi);
		    }
		    for(int cfei = 5; cfei < 10; cfei++)
		    {
		    	thetaCorFac.add(fdp.parameter(cfei-5).error(), seci, cfei);
		    }
		}
		...
		for(int seci = 0; seci < 6; seci++)
		{
			...
			System.out.println("a_0, a_1, a_2, a_3, a_4");
			   System.out.println(thetaCorFac.getItem(seci, 0) + ", "
				   					+ thetaCorFac.getItem(seci, 1) + ", "
				   					+ thetaCorFac.getItem(seci, 2) + ", "
				    				+ thetaCorFac.getItem(seci, 3) + ", "
				    				+ thetaCorFac.getItem(seci, 4));
		}
		
		for(int seci = 0; seci < 6; seci++)
		{
			System.out.println("======================================================================");
			System.out.println("Sec: " + (seci+1));
			System.out.println("#theta	#mu	#sigma");
			for(int thetabi = 0; thetabi < 9; thetabi++)
			{
				System.out.println(theta_ba[thetabi] + "	" + mean.getItem(seci, thetabi) + "	"
									+ sigma.getItem(seci, thetabi));
			}	
			System.out.println("======================================================================");
			System.out.println(" ");
		}
		...
		System.out.println("double[] thetagammaCorFac = new double[];");
		for(int seci = 0; seci < 6; seci++)
		{
			    System.out.println("double[] thetagammaCorFac_sec" + (seci+1) + " = new double[] {"
			    					+ thetaCorFac.getItem(seci, 0) + ", "
				   					+ thetaCorFac.getItem(seci, 1) + ", "
				   					+ thetaCorFac.getItem(seci, 2) + ", ");
			    System.out.println(	"								"+ thetaCorFac.getItem(seci, 3) + ", "
				    				+ thetaCorFac.getItem(seci, 4) + "};");
		}
	}
}