...
public class rgk_6535MeV_p_gamma_correction_quad_phi {
	...
	public static void histos_dp_vs_phi() {
		for(int thetabin = 0; thetabin < 9; thetabin++) {
			H2F hdp_vs_phi = new H2F("hdp_vs_phi", "hdp_vs_phi", 600, -150, 210, 200, 0.5, 2);
	...
	public static void histos_X_eg_m_vs_phi_bin() {
		for(int seci = 0; seci < 6; seci++) {
			for(int thetabin = 0; thetabin < 9; thetabin++) {
				for(int phibin = 0; phibin < 6; phibin++) {
	...
	public static void histos_dp() {
		for(int ihistsectordp = 0; ihistsectordp < 6; ihistsectordp++) {
			for(int ihistthetadp = 0; ihistthetadp < 9; ihistthetadp++) {
				for(int ihistphidp = 0; ihistphidp < 6; ihistphidp++) {
					H1F hdp = new H1F("hdp", "hdp", 200, 0.5, 2);
	...
	if(event.hasBank("REC::Particle") && event.hasBank("REC::Track") && event.hasBank("REC::Traj"))
	{
		...
		LorentzVector cor_p_gamma = new LorentzVector(0, 0, 0, 0);
		byte gamma_sector = 0;
				...
				if(event.hasBank("REC::Calorimeter"))
				{
					...
					for(int l = 0; l < reccal.rows(); l++)
					{
						...
						if(pindex == gamma_pindex && detector == 7 && layer == 1)
						{
							...
							double[] b2 = thetagammaCorFac.getItem(gamma_sector, 1);
							...
							double a2 = (b2[4]*theta_deg_gamma*theta_deg_gamma*theta_deg_gamma*theta_deg_gamma)+(b2[3]*theta_deg_gamma*theta_deg_gamma*theta_deg_gamma)
											+(b2[2]*theta_deg_gamma*theta_deg_gamma)+(b2[1]*theta_deg_gamma)+b2[0];
							...
							pcf = (a2*phi_rot_deg_gamma*phi_rot_deg_gamma)+(a1*phi_rot_deg_gamma)+a0;
							cor_p_gamma = new LorentzVector(pcf*p_gamma.px(), pcf*p_gamma.py(), pcf*p_gamma.pz(),
				...
				&& Q2 > 1 && W > 2)
		{
			...
			phi_rot_bnd.add(new double[] {-30, -6, -3, 0, 3, 6, 30}, 0);
			phi_rot_bnd.add(new double[] {-30, -10, -5, 0, 5, 10, 30}, 1);
			phi_rot_bnd.add(new double[] {-30, -12, -6, 0, 6, 12, 30}, 2);
			phi_rot_bnd.add(new double[] {-30, -13, -6, 0, 6, 13, 30}, 3);
			phi_rot_bnd.add(new double[] {-30, -15, -8, 0, 8, 15, 30}, 4);
			phi_rot_bnd.add(new double[] {-30, -16, -8, 0, 8, 16, 30}, 5);
			phi_rot_bnd.add(new double[] {-30, -17, -9, 0, 9, 17, 30}, 6);
			phi_rot_bnd.add(new double[] {-30, -18, -10, 0, 10, 18, 30}, 7);
			phi_rot_bnd.add(new double[] {-30, -20, -12, 0, 12, 20, 30}, 8);
		//	double[] phi_rot_bnd = new double[] {-30, -10, 0, 10, 30};
	...
	public static void main(String[] args) {
		...
		phi_rot_ba.add(new double[] {-8.48, -4.60, -1.51, 1.51, 4.60, 8.48}, 0);
		phi_rot_ba.add(new double[] {-11.94, -7.21, -2.40, 2.40, 7.21, 11.94}, 1);
		phi_rot_ba.add(new double[] {-14.27, -8.55, -2.85, 2.85, 8.55, 14.27}, 2);
		phi_rot_ba.add(new double[] {-16.29, -9.93, -3.33, 3.33, 9.93, 16.29}, 3);
		phi_rot_ba.add(new double[] {-17.99, -11.12, -3.82, 3.82, 11.12, 17.99}, 4);
		phi_rot_ba.add(new double[] {-19.24, -12.24, -4.22, 4.22, 12.24, 19.24}, 5);
		phi_rot_ba.add(new double[] {-20.39, -13.31, -4.79, 4.79, 13.31, 20.39}, 6);
		phi_rot_ba.add(new double[] {-21.14, -13.79, -4.90, 4.90, 13.79, 21.14}, 7);
		phi_rot_ba.add(new double[] {-22.86, -16.50, -6.63, 6.63, 16.50, 22.86}, 8);
		
		IndexedList<double[]> thetagammaCorFac = new IndexedList<>(2);
		double[] thetagammaCorFac_sec1_b0 = new double[] {1.0731725141266328, -0.0027560522306951177, -1.0309070561227802E-4,
															1.6194432098205592E-5, -2.3223908580420123E-7};
		double[] thetagammaCorFac_sec1_b1 = new double[] {-0.007820881847378, 0.002226311268147191, -2.1527381561765383E-4,
															8.29408886919926E-6, -1.0926890909912645E-7};
		double[] thetagammaCorFac_sec1_b2 = new double[] {1.932243162361816E-4, -3.6667594572543875E-5, 1.8492909483090351E-6,
															-4.107661178221302E-9, -8.081669609911507E-10};
		double[] thetagammaCorFac_sec2_b0 = new double[] {1.1746248756995399, -0.03834124453603896, 0.0029663996270566733,
															-7.210314726018207E-5, 4.766769735365334E-7};
		double[] thetagammaCorFac_sec2_b1 = new double[] {0.005799333235256986, -0.0016522285409266535, 1.5625637408790179E-4,
															-6.212164028154821E-6, 8.895502765916216E-8};
		double[] thetagammaCorFac_sec2_b2 = new double[] {2.8618395492660965E-4, -9.87284316174003E-5, 1.175120386126109E-5,
															-5.477693748277242E-7, 8.631900569273647E-9};
		double[] thetagammaCorFac_sec3_b0 = new double[] {1.3300261425451612, -0.08622121276902024, 0.008163518619486958,
															-3.029124403940682E-4, 3.988355211996828E-6};
		double[] thetagammaCorFac_sec3_b1 = new double[] {-5.6298498826947923E-5, -2.9919383351998684E-5, -8.130715679930835E-7,
															3.098477437993387E-8, 1.8015337604705656E-9};
		double[] thetagammaCorFac_sec3_b2 = new double[] {-5.61274016689697E-4, 1.591603727878759E-4, -1.525384896363333E-5,
															6.034867159152244E-7, -8.350806710346378E-9};
		double[] thetagammaCorFac_sec4_b0 = new double[] {1.0642069860846652, -0.01062453832145887, 0.0014528740067199814,
															-5.7229488794198676E-5, 7.583706618774157E-7};
		double[] thetagammaCorFac_sec4_b1 = new double[] {-0.003383429041694734, 9.218371080067852E-4, -1.0246828177391473E-4,
															4.753379474893567E-6, -7.714874088587042E-8};
		double[] thetagammaCorFac_sec4_b2 = new double[] {3.073825276273647E-4, -6.522858909291917E-5, 4.582190635044047E-6,
															-1.2314404629795097E-7, 1.0256624794299545E-9};
		double[] thetagammaCorFac_sec5_b0 = new double[] {1.186318990434752, -0.0483177907497191, 0.0050545262691357624,
															-1.973019648048968E-4, 2.7273723775767998E-6};
		double[] thetagammaCorFac_sec5_b1 = new double[] {0.0035405378510414702, -9.541998039773071E-4, 8.258616219362356E-5,
															-3.0972248103700134E-6, 4.1232552852604034E-8};
		double[] thetagammaCorFac_sec5_b2 = new double[] {-7.577059997769109E-4, 2.242566985412632E-4, -2.2358156811496898E-5,
															9.174875713556637E-7, -1.3162185681447683E-8};
		double[] thetagammaCorFac_sec6_b0 = new double[] {1.350112600523789, -0.0855954884364172, 0.008081346936584912,
															-2.9957994788761314E-4, 3.924993976343129E-6};
		double[] thetagammaCorFac_sec6_b1 = new double[] {0.006189513273980188, -0.001778269095021314, 1.6461551219186348E-4,
															-6.099820897433677E-6, 7.846321912872965E-8};
		double[] thetagammaCorFac_sec6_b2 = new double[] {1.3098052180830896E-4, -1.9273114144459326E-5, 4.6758734011673655E-7,
															2.6404996425423935E-8, -8.665285801267918E-10};
		thetagammaCorFac.add(thetagammaCorFac_sec1_b0, 1, 0);
		thetagammaCorFac.add(thetagammaCorFac_sec1_b1, 1, 1);
		thetagammaCorFac.add(thetagammaCorFac_sec1_b2, 1, 2);
		thetagammaCorFac.add(thetagammaCorFac_sec2_b0, 2, 0);
		thetagammaCorFac.add(thetagammaCorFac_sec2_b1, 2, 1);
		thetagammaCorFac.add(thetagammaCorFac_sec2_b2, 2, 2);
		thetagammaCorFac.add(thetagammaCorFac_sec3_b0, 3, 0);
		thetagammaCorFac.add(thetagammaCorFac_sec3_b1, 3, 1);
		thetagammaCorFac.add(thetagammaCorFac_sec3_b2, 3, 2);
		thetagammaCorFac.add(thetagammaCorFac_sec4_b0, 4, 0);
		thetagammaCorFac.add(thetagammaCorFac_sec4_b1, 4, 1);
		thetagammaCorFac.add(thetagammaCorFac_sec4_b2, 4, 2);
		thetagammaCorFac.add(thetagammaCorFac_sec5_b0, 5, 0);
		thetagammaCorFac.add(thetagammaCorFac_sec5_b1, 5, 1);
		thetagammaCorFac.add(thetagammaCorFac_sec5_b2, 5, 2);
		thetagammaCorFac.add(thetagammaCorFac_sec6_b0, 6, 0);
		thetagammaCorFac.add(thetagammaCorFac_sec6_b1, 6, 1);
		thetagammaCorFac.add(thetagammaCorFac_sec6_b2, 6, 2);
	...
	// DVCS	
		...
		*/
		...
		fpm = new F1D("fpm", "[amp]*gaus(x,[mean],[sigma])", (hX_eg_m.getMaximumBin()*0.02-0.45), (hX_eg_m.getMaximumBin()*0.02+0.25));
		...
		*/
		...
		fpm = new F1D("fpm", "[amp]*gaus(x,[mean],[sigma])", (hpcor_X_eg_m.getMaximumBin()*0.02-0.45), (hpcor_X_eg_m.getMaximumBin()*0.02+0.25));
		...
		*/
		...
		for(int seci = 0; seci < 6; seci++){
			...
			fpm = new F1D("fpm", "[amp]*gaus(x,[mean],[sigma])", (histGroups_X_eg_m_sec.getItem(seci).getMaximumBin()*0.02-0.45),
							(histGroups_X_eg_m_sec.getItem(seci).getMaximumBin()*0.02+0.25));
		...
		for(int seci = 0; seci < 6; seci++){
			...
			fpm = new F1D("fpm", "[amp]*gaus(x,[mean],[sigma])", (histGroups_p_cor_X_eg_m_sec.getItem(seci).getMaximumBin()*0.02-0.45),
							(histGroups_p_cor_X_eg_m_sec.getItem(seci).getMaximumBin()*0.02+0.25));
		...
		for(int seci = 0; seci < 6; seci++)
		{
			...
			{
				for(int phi_bini = 0; phi_bini < 6; phi_bini++)
				{
		...
		for(int theta_bini = 0; theta_bini < 9; theta_bini++)
		{
			...
			for(int phi_bini = 0; phi_bini < 6; phi_bini++)
			{
				...
				F1D f00dp = new F1D("f00dp", "[amp]*gaus(x,[mean],[sigma])",
									0.5+(histGroups_dp.getItem(setsector-1, theta_bini, phi_bini).getMaximumBin()*(2-0.5)/200)-0.2,
									0.5+(histGroups_dp.getItem(setsector-1, theta_bini, phi_bini).getMaximumBin()*(2-0.5)/200)+0.175);
				f00dp.setParameter(0, histGroups_dp.getItem(setsector-1, theta_bini, phi_bini).getMax());
				f00dp.setParameter(1, 0.5+(histGroups_dp.getItem(setsector-1, theta_bini, phi_bini).getMaximumBin()*(2-0.5)/200));
				f00dp.setParameter(2, 0.1);
		...
		*/
		...
		for(int secdpi = 0; secdpi < 6; secdpi++)
		{
			for(int theta_bini = 0; theta_bini < 9; theta_bini++)
			{
				...
				{
					F1D f00dp = new F1D("f00dp", "[amp]*gaus(x,[mean],[sigma])",
										0.5+(histGroups_dp.getItem(secdpi, theta_bini, phi_bini).getMaximumBin()*(2-0.5)/200)-0.2,
										0.5+(histGroups_dp.getItem(secdpi, theta_bini, phi_bini).getMaximumBin()*(2-0.5)/200)+0.175);
					f00dp.setParameter(0, histGroups_dp.getItem(secdpi, theta_bini, phi_bini).getMax());
					f00dp.setParameter(1, 0.5+(histGroups_dp.getItem(secdpi, theta_bini, phi_bini).getMaximumBin()*(2-0.5)/200));
					f00dp.setParameter(2, 0.1);
		...
		for(int seci = 0; seci < 6; seci++)
		{
			for(int thetabi = 0; thetabi < 9; thetabi++)
			{
				...
				for(int phibi = 0; phibi < 6; phibi++)
				{
					if(mean.getItem(seci, thetabi, phibi) > 0.75 && mean.getItem(seci, thetabi, phibi) < 1.25
							&& Math.abs(sigma.getItem(seci, thetabi, phibi)) < 0.2)
					{
				...
				FCN2 theFCN2 = new FCN2(phibci, meani, sigmai);
			    ...
			    {
			    	phiCorFac.add(min.userParameters().error(cfei-3), seci, thetabi, cfei);
			    }
			    phiCorFac.add(Math.sqrt(min.fval()), seci, thetabi, 6);
			}
		}
		...
		for(int seci = 0; seci < 6; seci++)
		{
			...
			{
				for(int phidpi = 0; phidpi < 6; phidpi++)
				{
				...
				{
					{
						...
						cfei.add(phiCorFac.getItem(seci, thetabfi, phcfi+3));
					}
				}
			...
			for(int cci = 0; cci < 3; cci++)
			{
				...
				for(int thetabgci = 0; thetabgci < 9; thetabgci++)
				{
					hhecf.addPoint(theta_ba[thetabgci], phiCorFac.getItem(seci, thetabgci, cci),
									0, phiCorFac.getItem(seci, thetabgci, cci+3));
				}
			}
		}
		
		for(int framehgecfi = 0; framehgecfi < 3; framehgecfi++){
			...
			for(int seci = 0; seci < 6; seci++)
			{
				...
				if(framehgecfi == 1) canhgecf.getPad(seci).setAxisRange(5, 35, -0.001, 0.001);
				if(framehgecfi == 2) canhgecf.getPad(seci).setAxisRange(5, 35, -0.0001, 0.0001);
			...
			for(int phcfi = 0; phcfi < 3; phcfi++)
			{
				...
				for(int thetabfi = 0; thetabfi < 9; thetabfi++)
				{
					System.out.println(theta_ba[thetabfi] + "	" + phiCorFac.getItem(seci, thetabfi, phcfi) + "	"
										+ phiCorFac.getItem(seci, thetabfi, phcfi+3));
				}
			}
			System.out.println(" ");
		}
